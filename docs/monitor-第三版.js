...
function buildTLS13ClientHello(hostname, keyShare) {
  ...
  // Cipher Suites (TLS 1.3) - 仅声明实际支持的密码套件
  p.push(0x00, 0x02); // 1 个密码套件，2 字节
  p.push(0x13, 0x01); // TLS_AES_128_GCM_SHA256 (完全支持)
  // 注意：虽然可以声明更多套件，但当前实现只能处理 AES-128-GCM
  // 如需支持其他套件（AES-256/ChaCha20/CCM），需要扩展密钥派生和解密逻辑
