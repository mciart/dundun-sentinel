...
export async function handleMonitor(env, ctx, forceWrite = false) {
  ...
  // 3. SSL 终极检测（轮询模式）
  // 由于采用轮询机制，每次只检测 1 个站点，CPU 消耗很低
  // 可以每次 Cron 都执行，快速完成所有站点的轮询
  const SSL_CHECK_INTERVAL = state.config.checkInterval * 60 * 1000; // 使用监控间隔
...
/**
 * SSL 证书检测 - 轮询模式（适配 Cloudflare Workers 免费版 CPU 限制）
 * 每次只检测 1 个站点，通过 state.lastSslCheckIndex 记录进度
 * 这样可以将多个站点的 CPU 消耗均摊到多次 Cron 触发中
 */
async function runUltimateSSLCheck(env, ctx, state) {
  ...
  // 初始化索引（兼容旧数据）
  if (state.lastSslCheckIndex === undefined) {
    state.lastSslCheckIndex = 0;
  }

  // 如果索引超出范围，重置为 0（开始新一轮）
  if (state.lastSslCheckIndex >= httpSites.length) {
    state.lastSslCheckIndex = 0;
  }

  // 🎯 核心优化：每次只检测 1 个站点
  const currentIndex = state.lastSslCheckIndex;
  const targetSite = httpSites[currentIndex];
  
  console.log(`🔒 SSL 轮询检测 [${currentIndex + 1}/${httpSites.length}]: ${targetSite.name}`);

  try {
    const hostname = new URL(targetSite.url).hostname;
    let info = null;
    let engine = 'TLS 1.2 (Raw)';

    // 1. 优先尝试 TLS 1.3 (Native X25519) - 总超时 15 秒
    try {
      info = await withTimeout(
        checkTLS13(hostname),
        15000,
        `TLS 1.3 total timeout for ${hostname}`
      );
      engine = 'TLS 1.3 (Native)';
    } catch (e13) {
      console.log(`[${targetSite.name}] TLS 1.3 失败: ${e13.message}`);
      // 2. 失败则降级到 TLS 1.2 (Raw TCP) - 总超时 15 秒
      try {
        info = await withTimeout(
          checkTLS12(hostname),
          15000,
          `TLS 1.2 total timeout for ${hostname}`
        );
      } catch (e12) {
    ...
    if (info) {
      ...
      const prevCert = targetSite.sslCert;
      targetSite.sslCert = info;
      targetSite.sslCertLastCheck = Date.now();
      
      const inc = handleCertAlert(state, targetSite, prevCert, info);
      if (inc && state.config?.notifications?.enabled) {
        ctx && ctx.waitUntil(sendNotifications(env, inc, targetSite, state.config.notifications));
      }
    }
  } catch (e) {
    const isTimeout = e.message.includes('timeout');
    if (isTimeout) {
      console.warn(`⏱️  [${targetSite.name}] SSL 检测超时 (${e.message}) - 将在下次重试`);
    } else {
      console.error(`❌ [${targetSite.name}] SSL 握手失败:`, e.message);
    }
  }

  // 🔄 更新索引，准备下次检测下一个站点
  state.lastSslCheckIndex = currentIndex + 1;
  state.lastSslCheck = Date.now();
  
  // ⚠️ 重要：立即保存状态，确保进度不丢失
  await updateState(env, state);
  console.log(`💾 SSL 检测进度已保存: 下次将检测第 ${state.lastSslCheckIndex + 1} 个站点`);
}
 ...
 * @returns {Promise}
 */
function withTimeout(promise, timeoutMs, errorMsg = 'Operation timeout') {
  return Promise.race([
    promise,
...
async function checkTLS13(hostname) {
  ...
  try {
    ...
    // 1. 解析 ServerHello (明文)
    // Record Header (5 bytes) - 添加 10 秒超时
    const shHeader = await withTimeout(
      rq.readBytes(5),
      10000,
      `ServerHello timeout for ${hostname}`
    );
...
async function checkTLS12(hostname) {
  ...
  try {
    ...
    // 按 TLS record 结构读取
    while (totalRead < maxBytes && loopCount < maxLoops) {
      loopCount++;
      // 读取 record header (5 bytes) - 添加 5 秒超时
      let header;
      try {
        header = await withTimeout(
          rq.readBytes(5),
          5000,
          `TLS12 header timeout`
        );
      } catch {
        break; // EOF 或超时
      }
      ...
      // 读取 record body - 添加 5 秒超时
      let body;
      try {
        body = await withTimeout(
          rq.readBytes(recLen),
          5000,
          `TLS12 record body timeout for ${hostname}`
        );
      } catch {
        break;
      }
...
function buildTLS13ClientHello(hostname, keyShare) {
  ...
  // Cipher Suites (TLS 1.3) - 增加更多套件提高兼容性
  p.push(0x00, 0x08); // 4 个密码套件，每个 2 字节 = 8 字节
  p.push(0x13, 0x01); // TLS_AES_128_GCM_SHA256 (推荐)
  p.push(0x13, 0x02); // TLS_AES_256_GCM_SHA384
  p.push(0x13, 0x03); // TLS_CHACHA20_POLY1305_SHA256
  p.push(0x13, 0x04); // TLS_AES_128_CCM_SHA256
