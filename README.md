...
轻量级网站监控系统 | 基于腾讯云 EdgeOne | 完全免费 | 边缘函数

[![License](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![EdgeOne](https://img.shields.io/badge/EdgeOne-Edge%20Functions-blue)](https://cloud.tencent.com/product/teo)
...
| 更改 | 说明 |
|------|------|
| **迁移至 EdgeOne** | 项目迁移至腾讯云 EdgeOne，使用 EdgeOne KV 存储数据 |
| **边缘函数支持** | 使用 EdgeOne Edge Functions，支持全球边缘节点部署 |
...
2. 创建新的 KV 命名空间（如 `dundun-sentinel-kv`）
3. 记录命名空间 ID

#### 第四步：部署边缘函数

1. 在 **边缘函数** 中创建新函数
2. 上传 `functions/` 目录中的函数代码
3. 绑定 KV 命名空间到环境变量 `MONITOR_DATA`
4. 配置路由规则

#### 第五步：部署前端静态资源

1. 构建前端：`cd frontend && npm install && npm run build`
2. 将 `frontend/dist` 目录的内容上传到 EdgeOne 静态托管

#### 第六步：配置定时任务

在 EdgeOne 控制台配置 Cron 触发器：
- 监控检测：`*/15 * * * *`（每15分钟）
- SSL证书检测：`0 4 * * *`（每天凌晨4点）
